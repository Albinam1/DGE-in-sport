...
library(annotate) # для платформы GPL570
library(ggrepel)
library(dplyr)
...
```{r}
# Предобработка и анализ с batch-коррекцией
process_gse <- funct?on(gse_data, gse_name, age_field_name) {
  eset <- gse_data  # основной набор
...
```{r}
# Тепловая карта отобранных генов
selected_genes <- c(
...
# Удалим гены, где logFC отсутствует во всех датасетах (если такие есть)
...
?oxplot(exprSet25941, outline = FALSE, las=2, main = "До нормализации", col= "lightblue")
```
...
ggplot(pcaData, aes(PC1, PC2, label=Sample)) +
    ...
    ggtitle("PCA по экспрессии генов") +    theme_minimal()

# Опционально: добав??ть метаданные (например, возраст, до/после тренировки), если они есть
pdata <- pData(gse_list["GSE25941"][[1]])
pcaData$group <- pdata$characteristics_ch1.1  # пример


            
# Тогда можно раскрасить точки по группам
ggplot(pcaData, aes(PC1, PC2, co?or=group)) + geom_point(size=3) + ggtitle("PCA с группами")
...
boxplot(exprSet28392, outline = FALSE, las=2, main = "До нормализации", col= "lightblue")
```
...
# Опционально: добавить метаданные (например, возраст, до/после тренировки), если они есть
pdata <- pData(gse_list["GSE28392"][[1]])
pcaData$group <- pdata$?haracteristics_ch1.2  # пример


            
# Тогда можно раскрасить точки по группам
ggplot(pcaData, aes(PC1, PC2, color=group)) + geom_point(size=3) + ggtitle("PCA с группами")
```

```{r}
exprSet59880 <- exprs(gse_list["GSE59880"][[1]])

boxplot(exprS?t59880, outline = FALSE, las=2, main = "До нормализации", col= "lightblue")
```
...
pcaData$group <- pdata$characteristics_ch1  # пример


            
# Тогда можно раскрасить точки по группам
ggplot(pcaData, aes(PC1, PC2, color=group)) + ge?m_point(size=3) + ggtitle("PCA с группами")
```
...
# --- ШАГ 2: Приведение матриц к общим генам (пересечение)
common_genes <- Reduce(intersect, lapply(expr_list, rownames))
expr_list <- lapply(expr_list, function(x) x[common_genes? ])

# --- ШАГ 3: Объединение данных по столбцам (образцам)
combined_expr <- do.call(cbind, expr_list)

# --- ШАГ 4: Создание вектора batch (откуда образец)
batch <- unlist(lapply(seq_along(expr_list), function(i) rep(names(expr_list)[i], ncol(expr_list[[i?]))))

# --- ШАГ 5: Проверка распределения данных и нормализация
boxplot(combined_expr, outline=FALSE, las=2, col="lightblue", main="До batch коррекции")

# Предполагается, что данные уже нормализованы (обычно GEOMatrix данные предобработаны)
# Если нет — ??римените RMA отдельно перед объединением

# --- ШАГ 6: Batch-коррекция с помощью ComBat
...
# -?- ШАГ 7: Визуализация PCA до и после batch коррекции
...
p1 <- plot_pca(combined_expr, batch, "PCA до batch коррекции")
p2 <- plot_pca(combat_expr, batch, "PCA после batch коррекции")
...
BiocManager::install("org.Hs.eg.db")  # Для человека
install.packages("tidyverse")
```
```{r}
...
# Используем GRCh37 (если нужна старая сборка)
